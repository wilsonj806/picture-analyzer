...
ImageData.prototype.parsePixels = () => {
...
ImageData.prototype.findMost = () => { // determine way to compare with the rest of the accumulator
...
ImageData.prototype.rgb2Hsl = () => {
  const pixels = this.imgData;
  for (let i = 0; i < pixels.data.length; i += 4) {
    const red = pixels.data[i + 0];
    const green = pixels.data[i + 1];
    const blue = pixels.data[i + 2];
...
ImageData.prototype.getLightness = () => {
